...
namespace JamesEngine
{
	...
	void Core::Run()
	{
		...
		// For GPU timing
		GLuint sceneTimeQuery[3] = { 0,0,0 };
		glGenQueries(3, sceneTimeQuery);
		int sceneQ = 0; // ring-buffer index [0..2]
		...
		while (mIsRunning)
		{
			...
			// Read back previous frame’s result to avoid stall
			int prev = (sceneQ + 2) % 3;
			GLuint available = 0;
			glGetQueryObjectuiv(sceneTimeQuery[prev], GL_QUERY_RESULT_AVAILABLE, &available);
			if (available)
			{
				GLuint64 ns = 0;
  ...
  //              splitFar[i] = lin + lambda * (log - lin);
	...
	void Core::RenderGUI()
	{
		//ScopedTimer timer("Core::RenderGUI");
